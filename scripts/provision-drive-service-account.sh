...
echo "  3. Per ADR-009, folder-level sharing alone will not expose per-file"
echo "     permission metadata (permissions.list() returns 403). If this"
echo "     client's Workspace admin can grant domain-wide delegation for"
echo "     $SA_EMAIL with the Drive readonly scope, set"
echo "     GOOGLE_DRIVE_DELEGATED_SUBJECT to the delegated user's email"
echo "     instead of relying on folder sharing alone."
